...
type User struct {
	...
	Email           string         `gorm:"column:email;size:255;not null;unique;unique;index;" json:"email" validate:"required,email,max=255,unique-field"`
	UserName        string         `gorm:"column:user_name;size:50;not null;unique;index;" json:"user_name" validate:"required,min=4,max=50,unique-field"`
	Password        string         `gorm:"column:password;size:255;not null" json:"-" validate:"required,min=6,max=50"`
	ConfirmPassword string         `gorm:"-" json:"-" validate:"required,min=6,max=50,eqfield=Password"`
